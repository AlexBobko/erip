package com.erip.reactiveupload;
...
/**
 * example to demonstrate fully reactive file upload using using SpringBoot2 WebFlux and NIO {@link
...
public class ReactiveUploadResource {
    ...
    private Logger LOGGER = LoggerFactory.getLogger(ReactiveUploadResource.class);
    private final GridFsTemplate gridFsTemplate;
    private final Parser parser;


    public ReactiveUploadResource(GridFsTemplate gridFsTemplate, Parser parser) {
    ...
    /**
     * upload handler method, mapped to POST. Like any file upload handler it consumes MULTIPART_FORM_DATA. Produces a
     * JSON respomse
     *
    ...
    /**
     * tske a {@link FilePart}, transfer it to disk using {@link AsynchronousFileChannel}s and return a {@link Mono}
    ...
    private Mono<String> saveFile(FilePart filePart) {
        ...
        //todo clear tmp folder when app down or start
        final String filePath = "tmp/".concat(filePart.filename());
//        final String filePath = "/tmp/".concat(filePart.filename());
...
//                        file = new File(filename);
                        if (file.exists()) {
                            parser.parse(file);
                            ObjectId id = gridFsTemplate.store(new FileInputStream(file), filename);
                            file.delete();
                        }


                    } catch (IOException ignored) {
                        LOGGER.error("closeCondition ", ignored);
                    }
                }
